  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose}>
        <div className='flex flex-col justify-center items-center my-8 mx-4'>
            ...
            <p className="text-brown-300 text-base font-garamond text-center md:w-[408px] font-normal py-2 ">By clicking 'Book Now' You will book the venue for the selected period. Please confirm the details below to reserve your booking.</p>
        </div>
        <div className="flex flex-row border-b-[1px] border-brown-100 justify-between w-full px-4 max-w-[500px]">
            ...
            <p className="pb-2 text-dark uppercase font-normal text-base">{venue.name}</p>
        </div>
        <div className="flex flex-row border-b-[1px] border-brown-100 justify-between w-full px-4 max-w-[500px]">
            <p className="text-brown-300 uppercase font-normal text-base">Date:</p>
            <p className="max-w-[200px] text-right pb-2 text-dark uppercase font-normal text-base"> {formatDate(selectedRange?.from)} - {formatDate(selectedRange?.to)}</p>
        </div>
        <div className="flex flex-row border-b-[1px] border-brown-100 justify-between w-full px-4 max-w-[500px]">
            <p className="text-brown-300 uppercase font-normal text-base">Guests:</p>
            <p className="pb-2 text-dark uppercase font-normal text-base">{guests}</p>
        </div>
        ...
        <p className="text-brown-300 text-base font-garamond text-center md:w-[408px] font-normal py-2 italic">Your venue will be visible on your profile page, where you can permanently remove it from your bookings.</p>
    </Modal>
  );
}
