...
function BookingGuestList({ venueId }) {
    ...
    if (!singleVenue?.bookings?.length) return <p> No bookings have been made yet </p>;
        ...
        <div className="overflow-x-auto pb-8 px-10">
            {singleVenue.bookings.map((booking) => (
                <table className="w-fit">
                    ...
                    <tbody>
                        <tr key={booking.id} className="border-t border-gray-300 font-caslon uppercase pt-5 tracking-wider text-left">
                            <td className="px-4 py-2">{booking.customer?.name}</td>
                            <td className="px-4 py-2"><span>{booking.dateFrom ? new Date(booking.dateFrom).toLocaleDateString() : "01/01/2025"}</span><span> - </span><span>{booking.dateFrom ? new Date(booking.dateTo).toLocaleDateString() : "01/01/2025"}</span></td>
