    ...
    (set, get) => ({
      ...
      removeFromBookings: async (id) => {
        try {
            ...
            showToast.bookingRemoved(booking?.id, booking?.venue?.name || 'Your booking');
        ...
        try {
            ...
            await Promise.all(
                currentBookings.map((b) => 
                    apiClient(`/holidaze/bookings/${b.id}`, { method: 'DELETE' }, true, true)
                )
            );
            set({
              bookings: [],
      ...
      getTotal: () =>
        get().bookings.reduce(
            (sum, b) => sum + (b.venue.price || 0 ) * (b.guests || 1), 0), 
